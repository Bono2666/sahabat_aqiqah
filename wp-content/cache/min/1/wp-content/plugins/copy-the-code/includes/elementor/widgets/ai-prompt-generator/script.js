(function($){const CTCAIPromptGenerator={init:function(){this._bind();this._generate()},_bind:function(){$(document).on('click','.ctc-ai-generator-button',this.doCopy);$(document).on('input','.ctc-ai-prompt-generator input',this.doChange)},copyToClipboard:async function(text){if(window.CTC&&window.CTC.CopyEngine){try{const copyEngine=new window.CTC.CopyEngine();const result=await copyEngine.execute({value:text});if(result.success){return!0}}catch(err){}}
if(navigator.clipboard&&navigator.clipboard.writeText){try{await navigator.clipboard.writeText(text);return!0}catch(err){}}
try{const textArea=document.createElement('textarea');textArea.value=text;textArea.style.cssText='position:fixed;opacity:0;pointer-events:none;';document.body.appendChild(textArea);textArea.select();const success=document.execCommand('copy');document.body.removeChild(textArea);return success}catch(err){return!1}},doChange:function(event){event.preventDefault();CTCAIPromptGenerator._generate()},doCopy:async function(event){event.preventDefault();const self=$(this)
