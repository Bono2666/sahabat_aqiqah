    ...
    class UltimatePostKit_Settings_API {
        ...
        function do_settings_fields($page, $section) {
            ...
            foreach ((array) $wp_settings_fields[$page][$section] as $field) {
                ...
                printf('<div class="bdt-option-item %1$s" data-widget-type="%2$s" data-content-type="%3$s" data-widget-name="%4$s">', $class, $field['args']['widget_type'], $field['args']['content_type'], strtolower($field['args']['name']));
        ...
        function callback_checkbox($args) {
            ...
            if ($plugin_name and $plugin_path) {
                ...
                if ($this->_is_plugin_installed($plugin_name, $plugin_path) and is_plugin_active($plugin_path)) {
                    ...
                    $html  .= sprintf('<input type="checkbox" class="checkbox" id="upk_%1$s[%2$s]" name="%1$s[%2$s]" value="on" %3$s />', $args['section'], $args['id'], checked($value, 'on', false));
                ...
                $html  .= sprintf('<label for="upk_%1$s[%2$s]">', $args['section'], $args['id']);
                $html  .= sprintf('<input type="hidden" name="%1$s[%2$s]" value="off" />', $args['section'], $args['id']);
                $html  .= sprintf('<input type="checkbox" class="checkbox" id="upk_%1$s[%2$s]" name="%1$s[%2$s]" value="' . $default_value . '" %3$s %4$s />', $args['section'], $args['id'], checked($value, $default_value, false), $pro_widget);
                $html    .= '<span class="switch"></span>';
                $html  .= '</label>';
                $html  .= '</fieldset>';
            }
        ...
        function callback_multicheck($args) {
            ...
            foreach ($args['options'] as $key => $label) {
                ...
                $html    .= sprintf('<label for="upk_%1$s[%2$s][%3$s]">', $args['section'], $args['id'], $key);
                $html    .= sprintf('<input type="checkbox" class="checkbox" id="upk_%1$s[%2$s][%3$s]" name="%1$s[%2$s][%3$s]" value="%3$s" %4$s />', $args['section'], $args['id'], $key, checked($checked, $key, false));
                $html    .= '<span class="switch"></span>';
        ...
        function callback_radio($args) {
            ...
            foreach ($args['options'] as $key => $label) {
                $html .= sprintf('<label for="upk_%1$s[%2$s][%3$s]">',  $args['section'], $args['id'], $key);
                $html .= sprintf('<input type="radio" class="radio" id="upk_%1$s[%2$s][%3$s]" name="%1$s[%2$s]" value="%3$s" %4$s />', $args['section'], $args['id'], $key, checked($value, $key, false));
                $html .= sprintf('%1$s</label><br>', $label);
            }
        ...
        function show_navigation() {
            ...
            foreach ($this->settings_sections as $tab) {
                $html .= sprintf('<li><a href="#%1$s" class="bdt-tab-item" id="bdt-%1$s" data-tab-index="%2$s">%3$s</a></li>', esc_attr($tab['id']), $count++, esc_html($tab['title']));
            }
        ...
        function show_forms() {
            ...
            foreach ($this->settings_sections as $form) {
                ...
                <div id="<?php echo esc_attr($form['id']); ?>_page" class="bdt-option-page">
                    ...
                    <div bdt-filter="target: .bdt-options" class="upk-options-parent" id="upk-options-parent-<?php echo esc_attr($i); ?>">
                            ...
                            <div class="bdt-widget-filter-wrapper bdt-grid">
                                ...
                                <div class="bdt-width-auto@m bdt-search-active-wrap bdt-flex bdt-flex-middle bdt-flex-between">
                                    <div class="bdt-widget-search">
                                        <input data-id="upk-options-parent-<?php esc_attr_e($i, 'ultimate-post-kit'); ?>" onkeyup="filterSearch(this);" bdt-filter-control="" class="bdt-search-input bdt-flex-middle" type="search" placeholder="Search widget..." autofocus>
                                    </div>
