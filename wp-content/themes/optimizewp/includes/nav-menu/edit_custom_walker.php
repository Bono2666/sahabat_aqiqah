...
class OptimizeMikado_Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu  {
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if ( ! empty( $item->_invalid ) ) {
            ...
            $title = sprintf( esc_html__( '%s (Invalid)', 'optimizewp' ), $item->title );
        } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
            $classes[] = 'pending';
            /* translators: %s: title of menu item in draft status */
            $title = sprintf( esc_html__('%s (Pending)', 'optimizewp'), $item->title );
        }
            ...
            <dt class="menu-item-handle">
                ...
                <span class="item-controls">
                    ...
                    <span class="item-order hide-if-js">
                        ...
                        ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'optimizewp'); ?>">&#8593;</abbr></a>
                        |
                        <a href="<?php
                        echo esc_url(wp_nonce_url(
                            add_query_arg(
                        ...
                        ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', 'optimizewp'); ?>">&#8595;</abbr></a>
                    </span>
                    <a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item', 'optimizewp'); ?>" href="<?php
                    ...
                    ?>"><?php esc_html_e( 'Edit Menu Item', 'optimizewp' ); ?></a>
                </span>
            </dt>
        </dl>
        ...
        <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
            <?php if( 'custom' == $item->type ) : ?>
                <p class="field-url description description-wide">
                    <label for="edit-menu-item-url-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'URL', 'optimizewp' ); ?><br />
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Navigation Label', 'optimizewp' ); ?><br />
                    <input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
                </label>
            </p>
            <p class="description description-thin">
                <label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Title Attribute', 'optimizewp' ); ?><br />
            ...
            <p class="field-link-target description">
                <label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
                    ...
                    <?php esc_html_e( 'Open link in a new window/tab', 'optimizewp' ); ?>
                </label>
            </p>
            <p class="field-css-classes description description-thin">
                <label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'CSS Classes (optional)', 'optimizewp' ); ?><br />
                    <input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
                </label>
            </p>
            <p class="field-xfn description description-thin">
                <label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Link Relationship (XFN)', 'optimizewp' ); ?><br />
                    <input type="text" id="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
                </label>
            </p>
            <p class="field-description description description-wide">
                <label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Description', 'optimizewp' ); ?><br />
                    <textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
                    <span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.', 'optimizewp'); ?></span>
                </label>
            </p>
            <?php
            ...
            <p class="field-custom description description-thin description-thin-custom">
                <label for="edit-menu-item-anchor-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Anchor', 'optimizewp' ); ?><br />
            ...
            <p class="field-custom description description-wide">
                ...
                <label for="edit-menu-item-nolink-<?php echo esc_attr($item_id); ?>">
                    ...
                    <?php esc_html_e( "Don't link", 'optimizewp' ); ?>
                </label>
            </p>
            <p class="field-custom description description-wide">
                <?php
                ...
                <label for="edit-menu-item-hide-<?php echo esc_attr($item_id); ?>">
                    ...
                    <?php esc_html_e( "Don't show", 'optimizewp' ); ?>
                </label>
            </p>
            <p class="field-custom description description-thin description-thin-custom">
                <label for="edit-menu-item-type-menu-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Type', 'optimizewp' ); ?><br />
                    <select class="widefat" id="edit-menu-item-type-menu<?php echo esc_attr($item_id); ?>" data-item-option data-name="menu_item_type_menu_<?php echo esc_attr($item_id); ?>">
                        ...
                        <option value="wide" <?php if($item->type_menu == "wide"){echo 'selected="selected"';} ?>><?php esc_html_e( 'wide', 'optimizewp' ); ?></option>
                        <option value="wide_icons" <?php if($item->type_menu == "wide_icons"){echo 'selected="selected"';} ?>><?php esc_html_e( 'wide with icons', 'optimizewp' ); ?></option>
                    </select>
                </label>
            </p>
            <p class="field-custom description description-thin description-thin-custom">
                <label for="edit-menu-item-wide-position-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Wide menu position', 'optimizewp' ); ?><br />
                    <select class="widefat" id="edit-menu-item-wide-position<?php echo esc_attr($item_id); ?>" data-item-option data-name="menu_item_wide_position_<?php echo esc_attr($item_id); ?>">
                        ...
                        <option value="left" <?php if($item->wide_position == "left"){echo 'selected="selected"';} ?>><?php esc_html_e( 'left', 'optimizewp' ); ?></option>
                        <option value="right" <?php if($item->wide_position == "right"){echo 'selected="selected"';} ?>><?php esc_html_e( 'right', 'optimizewp' ); ?></option>
                    </select>
                </label>
            </p>
	        ...
	        <p class="field-custom description description-wide">
		        <label for="edit-menu-item-background-image-<?php echo esc_attr($item_id); ?>">
			        <?php esc_html_e('Background image for Dropdown', 'optimizewp'); ?><br />
            ...
            if(is_array($iconCollections) && count($iconCollections)) {
                ...
                <p class="field-custom description description-thin description-thin-custom">
                    <label for="edit-menu-item-icon-pack-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Icon Pack', 'optimizewp' ); ?><br />
                        ...
                        <br/><?php esc_html_e( 'Only with "default" & "wide with icons" menu types', 'optimizewp' ); ?>
                    </label>
                </p>
                ...
                <p class="field-custom description description-thin description-thin-custom mkdf-icon-select-holder">
                    <label for="edit-menu-item-icon-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Icon', 'optimizewp' ); ?><br />
	        ...
	        <p class="field-custom description description-thin description-thin-custom" style="clear:both;">
		        <label for="edit-menu-item-featured-icon-<?php echo esc_attr($item_id); ?>">
			        <?php esc_html_e( 'Featured icon', 'optimizewp' ); ?><br />
			        <select class="widefat" id="edit-menu-item-featured-icon<?php echo esc_attr($item_id); ?>" data-item-option data-name="menu_item_featured_icon_<?php echo esc_attr($item_id); ?>">
				        ...
				        <option value="icon_star" <?php if($item->featured_icon == "icon_star"){echo 'selected="selected"';} ?>><?php esc_html_e('Star', 'optimizewp'); ?></option>
				        <option value="icon_heart " <?php if($item->featured_icon == "icon_heart "){echo 'selected="selected"';} ?>><?php esc_html_e('Heart', 'optimizewp'); ?></option>
				        <option value="arrow_right-up" <?php if($item->featured_icon == "arrow_right-up"){echo 'selected="selected"';} ?>><?php esc_html_e('Arrow', 'optimizewp'); ?></option>
			        </select>
		        </label>
	        </p>
            ...
            <p class="field-custom description description-wide">
                <label for="edit-menu-item-sidebar-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Custom widget area', 'optimizewp' ); ?><br />
                    ...
                    <br/><?php esc_html_e( 'Only with "wide & wide with icons" menu type', 'optimizewp' ); ?>
                </label>
            </p>
            <?php
            ...
            <div class="menu-item-actions description-wide submitbox">
                <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
                    <p class="link-to-original">
                        <?php printf( esc_html__('Original: %s', 'optimizewp'), '<a href="' . esc_url( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                    </p>
                <?php endif; ?>
                ...
                )); ?>"><?php esc_html_e('Remove', 'optimizewp'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
                ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'optimizewp'); ?></a>
            </div>
