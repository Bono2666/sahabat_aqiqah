...
class OptimizeMikadoFramework {
    ...
    private function __construct() {
        $this->mkdOptions = OptimizeMikadoOptions::get_instance();
        $this->mkdMetaBoxes = OptimizeMikadoMetaBoxes::get_instance();
    }
	...
	public function setSkin(OptimizeMikadoSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class OptimizeMikadoSkinManager
 *
...
class OptimizeMikadoSkinManager {
    ...
    private function setSkin($skinName = 'mikado') {
        if($skinName !== '') {
            if(file_exists(MIKADO_FRAMEWORK_ROOT_DIR.'/admin/skins/'.$skinName.'/skin.php')) {
                require_once MIKADO_FRAMEWORK_ROOT_DIR.'/admin/skins/'.$skinName.'/skin.php';
	
	            $this->skin = new OptimizeMikadoSkin();
            }
        } else {
            $this->skin = false;
        }
...
/**
 * Class OptimizeMikadoSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class OptimizeMikadoSkinAbstract {
    /**
     * @var string
     */
    protected $skinName;
    ...
    /**
     * Loads template part with params. Uses optimize_mikado_find_template_path function which is child theme friendly
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if($template !== '') {
            include(optimize_mikado_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
        }
    }
    ...
    public function setShortcodeJSParams() { ?>
		<script>
			window.mkdSCIcon = '<?php echo optimize_mikado_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
class OptimizeMikadoOptions {
    ...
    public function getOptionValue($key) {
        ...
        if(array_key_exists($key, $optimize_mikado_options)) {
            return $optimize_mikado_options[$key];
...
/*
   Class: OptimizeMikadoAdminPage
   A class that initializes Mikado Admin Page
*/
class OptimizeMikadoAdminPage implements iOptimizeMikadoLayoutNode {
    ...
    function __construct($slug="", $title_label="", $icon = "") {
        $this->layout = array();
        $this->factory = new OptimizeMikadoFieldFactory();
        $this->slug = $slug;
    ...
    public function renderChild(iOptimizeMikadoRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: OptimizeMikadoMetaBoxes
...
/*
   Class: OptimizeMikadoMetaBox
   A class that initializes Mikado Meta Box
*/
class OptimizeMikadoMetaBox implements iOptimizeMikadoLayoutNode {
	...
	public function setTitle($label) {
		...
		return $optimize_mikado_Framework->getSkin()->getSkinLabel().' '.$label;
 	}
}

global $optimize_mikado_Framework;

$optimize_mikado_Framework = OptimizeMikadoFramework::get_instance();
$mkdSkinManager = new OptimizeMikadoSkinManager();
$optimize_mikado_Framework->setSkin($mkdSkinManager->getSkin());
