...
if(!function_exists('optimize_mikado_get_title_text')) {
	...
	function optimize_mikado_get_title_text() {
		...
		if(is_tag()) {
			...
			$title = single_term_title("", false).esc_html__(' Tag', 'optimizewp');
		...
		elseif(is_author()) {
			...
			$title = esc_html__('Author:', 'optimizewp')." ".get_the_author();
		...
		elseif(is_404()) {
			...
			if(optimize_mikado_options()->getOptionValue('404_title') != "") {
				...
				$title = esc_html__('404 - Page not found', 'optimizewp');
			}
		...
		elseif(is_archive()) {
			$title = esc_html__('Archive', 'optimizewp');
...
if(!function_exists('optimize_mikado_subtitle_text')) {
	...
	function optimize_mikado_subtitle_text() {
		...
		print optimize_mikado_get_module_part($subtitle);
	}
}
	...
	function optimize_mikado_custom_breadcrumbs() {
		...
		if(is_home() && !is_front_page()) {
			...
			if(is_category() || optimize_mikado_is_product_category()) {
				...
				$output .= $before.esc_html__('Search results for ', 'optimizewp').'"'.get_search_query().'"'.$after;
				...
				$output .= $before.esc_html__('Posts tagged ', 'optimizewp').'"'.single_tag_title('', false).'"'.$after;

			} elseif(is_author()) {
				global $authordata;
				$output .= $before.esc_html__('Articles posted by ', 'optimizewp').$authordata->display_name.$after;
			...
			if(get_query_var('paged')) {
				...
				$output .= $before." (".esc_html__('Page', 'optimizewp').' '.get_query_var('paged').")".$after;
...
if(!function_exists('optimize_mikado_get_title_content_padding')) {
	...
	function optimize_mikado_get_title_content_padding() {
		$title_padding = apply_filters('optimize_mikado_title_content_padding', 0);
		
		return intval('optimize_mikado_title_content_padding');
	}
}
